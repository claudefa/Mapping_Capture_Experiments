...
do
	...
	jobName=${qu}/bwa_hg19_${sample}.sh
        ...
        echo "$BWA mem -t 4 -M $FASTA_ASSEMBLY $p1 $p2 | awk 'substr(\$0,1,1)==\"@\" || (\$9>= 35 && \$9<=500) || (\$9<=-35 && \$9>=-500)' | $SAMTOOLS view -Shb - | java -Djava.io.tmpdir=\${TMPDIR} -jar $PICARDTOOLS/AddOrReplaceReadGroups.jar I=/dev/stdin O=${OUTDIR}/${sample}_pe.bam SORT_ORDER=coordinate QUIET=TRUE COMPRESSION_LEVEL=9 MAX_RECORDS_IN_RAM=150000 RGLB=${sample} RGPL=Illumina RGPU=${sample} RGSM=${sample} RGID=${sample} VALIDATION_STRINGENCY=SILENT"> ${jobName}
	chmod 755 $jobName
