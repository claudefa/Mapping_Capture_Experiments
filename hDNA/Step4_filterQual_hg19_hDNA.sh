...
do
	sample=`echo $line | awk '{print $1}'`
        pool=`echo $sample | cut -d"_" -f3`
        library=`echo $sample | cut -d"_" -f1`
	...
	echo "samtools sort -T ${sample} -o $bamSorted $bamFile; \
	      ...
	      samtools index $bamFiltered; " > ${jobName}

	chmod 755 $jobName
	python3 ~/submit.py  -c $jobName -n FilChr21${sample} -o ${out}/FilCh${sample}.out -e  ${out}/FilCh${sample}.err -u 1 -w "0:59:00" 
